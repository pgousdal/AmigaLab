...
def _json(start, status, value):
    ...
    start(status, [("Content-Type", "application/json; charset=utf-8"), ("Content-Security-Policy", "default-src 'self'; object-src 'none'"), ("X-Content-Type-Options", "nosniff"), ("X-Frame-Options", "DENY"), ("Referrer-Policy", "no-referrer"), ("Content-Length", str(len(body)))])
    return [body]
...
def _html(start, status, title, body):
    ...
    start(status, [("Content-Type", "text/html; charset=utf-8"), ("Content-Security-Policy", "default-src 'self'; style-src 'unsafe-inline'; object-src 'none'"), ("X-Content-Type-Options", "nosniff"), ("X-Frame-Options", "DENY"), ("Referrer-Policy", "no-referrer"), ("Content-Length", str(len(content)))])
    return [content]
    ...
    def app(environ, start_response):
        ...
        try:
            ...
            if path.startswith("/catalog/documents/"):
                ...
                return _html(start_response, "Catalog document", "<h1>" + escape(value.get("title", value["id"])) + "</h1><pre>" + escape(json.dumps(value, indent=2, sort_keys=True)) + "</pre>")
